...
class BiometricDataset(Dataset):
    def __init__(self, base_path, num_people, transform_fp=None, transform_iris=None, augment=False):
        ...
        logger.info(f"Loading dataset from {base_path} for {num_people} people...")
        ...
        for person_id in range(1, num_people + 1):
            ...
            fp_imgs = glob.glob(os.path.join(fp_path, "*.bmp")) + glob.glob(os.path.join(fp_path, "*.BMP"))
            left_imgs = glob.glob(os.path.join(left_path, "*.bmp")) + glob.glob(os.path.join(left_path, "*.BMP"))
            right_imgs = glob.glob(os.path.join(right_path, "*.bmp")) + glob.glob(os.path.join(right_path, "*.BMP"))
            if fp_imgs and left_imgs and right_imgs:
                self.samples.append((fp_imgs[0], left_imgs[0], right_imgs[0], person_id - 1))
            else:
                skipped += 1
                logger.debug(f"Skipping person {person_id}: missing images")
        load_time = time.time() - start_time
        logger.info(f"Loaded {len(self.samples)} samples in {load_time:.2f}s (skipped {skipped})")
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            logger.error(f"Error loading images for sample {idx}: {e}")
            raise
...
def get_dataloaders(base_path, num_people, batch_size, num_workers=2, train_split=0.7, val_split=0.15, augment_train=True):
    logger.info(f"Creating dataloaders with splits: train={train_split}, val={val_split}")
    transform_fp, transform_iris = get_transforms(augment=False)
    full_dataset = BiometricDataset(base_path, num_people, transform_fp, transform_iris)
